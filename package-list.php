...
<?php 
$sql = "SELECT tp.*, sum(totall) AS total
FROM tbltourpackages tp
LEFT JOIN tblbooking tb ON tp.PackageId = tb.PackageId
LEFT JOIN tblmembers tm ON tp.PackageId = tm.package_id
...
if($query->rowCount() > 0)
{
	...
	{
		$max_reached = false;
		if ($result->total >= $result->number_of_people) {
			$max_reached = true;
		}
	?>
